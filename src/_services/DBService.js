...
export const createTable = async () => {
  await db.transaction((tx) => {
    ...
    tx.executeSql(
      "create table if not exists graph_data (login_user	TEXT,pwd_user	TEXT,url	TEXT,uidOu	TEXT,period TEXT,group_ind	TEXT,value	TEXT);"
    );
  });
...
export const saveUser = async (user) => {
  await createTable();
  //await console.log("==========save user==========" + JSON.stringify(user));
  await db.transaction((tx) => {
    tx.executeSql(
    ...
    db.transaction((tx) => {
      tx.executeSql(
        ...
        function (tx, results) {
          ...
          //console.log('==========results a voir 2==========',JSON.stringify(results))
    ...
    db.transaction((tx) => {
      tx.executeSql(
        ...
        function (tx, results) {
          let listUser = results.rows._array;
          let taille = listUser.length;
          //console.log("==========Last User a voir ==========",results.rowsAffected);
...
export const saveGraphData = async (graphData) => {
  await createTable();
  //await console.log("============graphData===========",JSON.stringify(graphData.value))

  await db.transaction((tx) => {
    tx.executeSql(
      "select * from graph_data where url=? and uidOu=? and period=? and group_ind=?;",
      [graphData.url, graphData.uidOu, graphData.period, graphData.group_ind],
      async (_, { rows }) => {
        ...
        if (rows.length > 0) {
          ...
          await db.transaction((tx) => {
            tx.executeSql(
              "update graph_data set value = ? where url=? and uidOu=? and period=? and group_ind=?;",
              [
          ...
          await db.transaction((tx) => {
            tx.executeSql(
              "insert into graph_data (url,uidOu,period,group_ind,value) values (?,?,?,?,?);",
              [
                graphData.url,
                graphData.uidOu,
                graphData.period,
