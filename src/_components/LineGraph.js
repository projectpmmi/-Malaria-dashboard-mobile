...
const LineGraph = (props) => {
  ...
  const graph = useSelector((state) => state.chart.chartData3);
  let data = graph ? graph.data : new Object();
    ...
    let htmlContent = `<html>
                        ...
                        <body style="text-align: center;">
                          <h1 style="font-size: 50px; font-family: Helvetica Neue; font-weight: normal;">
                            ${graph?.titre + " de " + graph?.orgunitName}
                          </h1>
  ...
  const option = {
    title: {
      text: graph?.titre + " de " + graph?.orgunitName,
