...
import { getMetadata, getOrgunitList } from "../_services/DataService";
  ...
  const manageNewData = async (user) => {
    ...
    await getMetadata(user.login, user.pwd, user.url)
      .then(async (response) => {
        ...
        await getOrgunitList(user.login, user.pwd, user.url)
          .then(async (orgunits) => {
            //console.log("========orgunit=========",JSON.stringify(orgunits))
            await saveOrgunit(user.url, orgunits);
            //props.addOrguit(orgunits);
            dispatch(setListOrgunit(orgunits));
          })
          .catch((error) => {
            console.log("========orgunit error=========", error);
          });
